...
# date:2019/04/19
# author:fish
# desc:query card's bank info from alipay
...
# query_url = 'http://www.cardcn.com/search?word='  # api接口
query_url = 'https://ccdcapi.alipay.com/validateAndCacheCardInfo.json?cardNo={}&cardBinCheck=true' # api接口
...
def query_from_mob():
    query_url = 'http://apicloud.mob.com/appstore/bank/card/query?key=yourkey&card={}'
    global result_str
    for account in accounts:
        result = requests.get(query_url.format(account)).json()
        if result['msg'] == 'success':
            data = result['result']
            result_str = result_str + account + tab + data['bank'] +tab + data['cardName'] + tab + data['cardType'] + next_line
            print('{}\t{}'.format(account, data['bank']))
        else:
            result_str = result_str + account + tab + result['msg'] + next_line
            print('{}\t{}'.format(account,result['msg']))
        # time.sleep(0.3)

if __name__ == '__main__':
    with open(account_file,'r') as af:
        accounts = af.read().splitlines()
        try:
            deal_data()
            # query_from_mob()
